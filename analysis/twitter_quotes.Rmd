...
if (!snapshot_loaded) {
  ...
  semantic_phrase <- "There is no cure for COVID-19."
  aspect1 <- "We should develop herd immunity"
  aspect2 <- "We should quarantine until there is a vaccine"
...
```{r, echo=FALSE}
###############################################################################
# Run K-means on all the quoted embedding vectors
###############################################################################
if (!snapshot_loaded) {
...
if (!exists("summaries.df") && isTRUE(summarize_clusters)) {
  ...
  #Summarize quoted tweets
  quoted_summaries.df <- summarize_tweet_clusters(quoted.vectors.df,
                                                  center_nn=summarize_center_nn,
                                                  max_len=summarize_max_len,
                                                  num_beams=summarize_num_beams,
                                                  temperature=summarize_temperature,
                                                  model=summarize_model,
                                                  summarizer_url=summarizer_url,
                                                  use_quoted_centers=TRUE)
  
  #update the cluster & subcluster center vectors' text with summaries
  quoted.vectors.df[rownames(quoted_summaries.df), "full_text"] <- quoted_summaries.df$summary
  
  #assign the summaries back to the cluster & subcluster lists
  invisible(
    mapply(function(vector_type, cluster, subcluster, summary, quoted_summary) {
      if (vector_type == "cluster_center") {
        clusters[[cluster]]$summary <<- summary
        clusters[[cluster]]$quoted_summary <<- quoted_summary
      } else {
        clusters[[cluster]]$subclusters[[subcluster]]$summary <<- summary
        clusters[[cluster]]$subclusters[[subcluster]]$quoted_summary <<- quoted_summary
      }
    }, summaries.df$vector_type, summaries.df$cluster, summaries.df$subcluster, summaries.df$summary, 
                                                                          quoted_summaries.df$summary))
}
```
...
###############################################################################
Map.numPercent = length(summary.tibble)/length(tweet.vectors.df)*100
if( isTRUE(show_overall_sentiment_map)) {
  geographical_sentiment_maps(summary.tibble,states,Map.numPercent)
}
```
...
#   #}
#   #master.tsne <- Rtsne(quoted.vectors.matrix, dims=tsne_dims, perplexity=tsne_clusters_perplexity, 
#   #                     max_iter=tsne_clusters_max_iter, check_duplicates=FALSE)
#   Y <- cbind(aspect1_cosine_similarity, aspect2_cosine_similarity)
#   master.tsne.plot <- prepare_tsne_plot_df(tsne_dims, Y, tweet.vectors.df, FALSE)
# }
# 
...
# title <- paste0("Master Plot: tweets similar to '", semantic_phrase, "'")
# taglist[[2]] <- plot_tweets(master.tsne.plot, title=title, sentiment_threshold=sentiment_threshold,
#                             type="clusters", mode=plot_mode, webGL=TRUE, xlabel=aspect1, ylabel=aspect2)
# 
# #Cluster plots
# plot_index <- 3
# for (i in 1:k) {
...
#     cluster.tsne.plot <- prepare_tsne_plot_df(tsne_dims, cluster.tsne$Y, 
#                                               tweet.vectors.df[master.tsne.plot$cluster == i,], 
#                                               TRUE)
...
#     title <- paste('Cluster', i, ":", clusters[[i]]$quoted_label, "(regrouped by subcluster)")
...
#     taglist[[plot_index]] <- htmltools::HTML(kable(clusters[[i]]$quoted_word_scores[1:5,], 
#                                                    caption=paste("Cluster", i, "word scores")) %>%
#                                                kable_styling())
#     plot_index <- plot_index + 1
#   }
#   
...
#     taglist[[plot_index]] <- htmltools::HTML(kable(clusters[[i]]$quoted_nearest_center[1:summarize_center_nn,], 
#                                                    caption=paste("Cluster", i, "nearest neighbors to center")) %>%
#                                                kable_styling())
#     plot_index <- plot_index + 1
#   }
#   
...
###############################################################################
tsne_dims =3
run_tsne <- (plot_mode=="3d" && tsne_dims==2) || (plot_mode=="2d" && tsne_dims==3)
if (run_tsne) {
  tsne_dims <- ifelse(plot_mode=="3d", 3, 2)
  tsne_coords <- c("X","Y")
...
```{r, echo=FALSE}
if (output_summaries_table) {
  summaries_table.df <- format_summaries_table(summaries.df, quoted_summaries.df)
