...
#Generate the plot
sscore_23<-sscore_fn(tweet.vectors.matrix, 1)
```
...
sscore_fn<- function(data,seed){
  ...
  y<-future_lapply(X,function(x){
    ...
    km <- kmeans(data, centers=x, iter.max=30)
    SIL<- silhouette(km$cluster, dist(data))
    wss[i-fc+1,2]<-new_mean<- mean(SIL[, 3])
    return(new_mean)
  })
}
```
...
```{r, echo=FALSE}
sscore_test<-sscore_fn(tweet.vectors.matrix,1)
print(sscore_test)
```
...
sscore11<-c(sscore11,sscore_fn(tweet.vectors.matrix,1))




sscore22<-c(sscore22,sscore_fn(tweet.vectors.matrix,2))
sscore33<-c(sscore33,sscore_fn(tweet.vectors.matrix,3))
sscore44<-c(sscore44,sscore_fn(tweet.vectors.matrix,4))
```
