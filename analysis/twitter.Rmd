...
perform_divisiveness_linreg <- TRUE
```
...
if (!snapshot_loaded) {
  ...
  for (i in 1:k) {
    ...
    cluster.label <- get_label(cluster.word_scores, top_k=cluster_label_top_k)
    ...
    for (j in 1:cluster.k) {
      ...
      subcluster.word_scores <- subcluster.word_freqs %>%
        ...
        mutate(score = count.x * log(count.x / count.y)) %>%
        dplyr::arrange(desc(score)) 
      subcluster.label <- get_label(subcluster.word_scores, top_k=subcluster_label_top_k)
...
```{r, echo=FALSE}
if (isTRUE(perform_divisiveness_linreg)) {
  ...
  kable(divisiveness.df) %>% kable_styling()
}
```
