...
# and neutral to negative below zero.
...
resultfields <- '"user.screen_name", "user.verified", "user.location", "place.full_name", "place.country", "text", "full_text", "extended_tweet.full_text", "embedding.use_large.primary", "created_at"'
...
required_fields <- c("full_text", "user_screen_name", "user_verified", "user_location", "place.country", "place.full_name", "sentiment", "created_at")
...
tweet.vectors.df <- tweet.vectors.df[, c("full_text", "user_screen_name", "user_verified", "user_location", "user_location_type", "sentiment", "created_at")]
...
master.word_freqs <- master.word_freqs %>%
...
for (i in 1:k) {
  ...
  cluster.label <- get_label(cluster.word_freqs, top_k=cluster_label_top_k)
  ...
  for (j in 1:cluster.k) {
    ...
    subcluster.word_freqs <- get_word_freqs(subcluster.df$full_text) %>%
      ...
      mutate(score = count.x * log(count.x / count.y)) %>%
      dplyr::arrange(desc(score)) 
    subcluster.label <- get_label(subcluster.word_freqs, top_k=subcluster_label_top_k)
