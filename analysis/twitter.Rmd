...
if (!snapshot_loaded) {
  ...
  embedder_url <- "http://localhost:8008/embed"
...
###############################################################################
if (!snapshot_loaded) {
  resultfields <- sprintf('"id_str", "created_at", "user.screen_name", "user.verified", "user.location","normalized_state", "place.full_name", "place.country", "text", "full_text", "extended_tweet.full_text", "embedding.%s.primary"', embedding_type)
  ...
  tweet.vectors.matrix <- t(simplify2array(results$df[,sprintf("embedding.%s.primary", embedding_type)]))
}
```
