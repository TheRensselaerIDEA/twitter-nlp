...
show_overall_sentiment <- TRUE
show_cluster_sentiment <- TRUE
show_subcluster_sentiment <- FALSE
...
graph_shape <- "stack" # "dodge" for grouped, "stack" for stacked
```
...
```{r, echo=FALSE}
###############################################################################
# Run plots showing sentiment over time. This can be done for the entire sample, or
# comparatively for each high-level cluster. Sentiment has been categorized to help
# with visualization. Divisiveness over time as well as comparison inside the high-
# level clusters have not been implemented yet.
###############################################################################

# overall sentiment plot
if (isTRUE(show_overall_sentiment)) {
  ...
  summary.df <- data.frame(week = rep(c(1:num_weeks), each=3), sentiment = factor(rep(c("positive", "neutral", "negative"), num_weeks), ordered=TRUE), count = 0)
  ...
  ggplot(summary.df, aes(x = week, y = count, fill=sentiment)) + 
      ...
      coord_cartesian(xlim = c(10, num_weeks + 1)) +
...
if (isTRUE(show_cluster_sentiment)) {
  ...
  ggplot(summary.df, aes(x = week, y = count, fill = sentiment_mean)) + 
      ...
      scale_fill_gradient2(name = "Sentiment Average", limits = c(-0.5,0.5), low = "red", mid = "white", high = "blue", midpoint = 0) +
      ggtitle("Tweets by Week") + 
      ylab("Tweet Count") +
      theme(axis.title.x = element_blank()) +
