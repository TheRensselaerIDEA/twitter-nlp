...
# Load Model
model = BartForConditionalGeneration.from_pretrained(config.transformers_model)
tokenizer = BartTokenizer.from_pretrained(config.transformers_model)
...
def summarize(original_text, max_len, num_beams, temperature, batch_size):
...
class BatchSummarizer(Resource):
    def post(self):
        ...
        summaries = summarize(text, max_len, num_beams, temperature, config.batch_size)
        return summaries
