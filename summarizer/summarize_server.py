...
def start():
    ...
    def summarize(original_text, max_len, num_beams, temperature, do_sample, batch_size, model_name):
    ...
    batch_reqparser.add_argument("do_sample", type=bool, action="store_true", default=True, required=False)
    ...
    class BatchSummarizer(Resource):
        def post(self):
            ...
            do_sample = reqargs["do_sample"]
            model = reqargs["model"]
            summaries = summarize(text, max_len, num_beams, temperature, do_sample, config.batch_size, model)
            return summaries
