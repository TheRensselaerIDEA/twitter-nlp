...
source("~/COVID-Twitter/analysis/Elasticsearch.R")
source("~/COVID-Twitter/analysis/plot_tweet_timeseries.R")
```
...
<!-- If this notebook has problems knitting, I recommend placing it in /COVID-Twitter/analysis and knitting it there. In such a case, also move the images folder-->
...
```
  ...
  * **Academic Paper Contributions**: For writing the project paper for publication, I am currently contributing with visualizations specific for the paper, the appendix table and wrting like all other teammates.
  In particular, I have worked on making specific visualizations for the cluster divisiveness over time using both bar plots and line plots, based on work previously done for `plot_tweet_timeseries`, shown in the screenshots below.
  ![Image 1](campoh_images/fig-1.png)
  ![Image 2](campoh_images/fig-2.png)
  Furthermore, I'm also compiling all the DistilBART summaries into an appendix table, working in tandem with the team on interpreting the clusters to select for the final analysis and writing about the sections of the project I am most familiar with.
...
* [`twitter.Rmd`](https://github.com/TheRensselaerIDEA/COVID-Twitter/blob/master/analysis/twitter.Rmd): In the last week of the project, I worked on incorporating keyword labeling and the paper specific plots into `twitter.Rmd` (#29). 
...
The computation of the divisiveness score is described below.
  ...
  Let $\vec{s}$ be a vector of discretized tweet sentiments, i.e. $\vec{s}_i \in \{-1, 0, 1\}$ such that the skewness $\gamma$ has mean $\mu_\gamma$ and variance $\sigma^2_\gamma$ and the kurtosis $\kappa$ has mean $\mu_\kappa$ and variance $\sigma^2_\kappa$.
...
```
  ...
  * **Discrete vs Continuous Sentiment**: When working with sentiment, the approaches of using the continuous VADER score or a discretized sentiment class using the $-0.05$ and $0.05$ threshholds are both advantageous in different contexts. 
  Working with a continuous metric of course has many advantages and greatly simplifies some calculations and analyses, however, VADER in particular has a tendency to assign most scores near 0, which often may paint the picture of most tweets being neutral when in fact they are positive or negative. 
  By using the thresholds, we get a fundamentally different picture, where most tweets are either positive or negative and a minority are neutral, which is consistent with direct inspections of the data. 
  
    For example, I've found that using the discrete sentiment in the divisiveness score gives results that are much more coherent and consistent with the data, with sentiment distributions always being bimodal and groups of tweets always being divisive but different groups having different levels or trends in divisiveness over time.
  Meanwhile, when talking, say, cluster average sentiment, using the continuous scores gives higher weight to positive and negative sentiment, so that clusters are less likely to be neutral in comparasion to a discretized average.
  I would encourage anyone following up with the project to carefully consider the impacts of using continuous and discrete sentiment in their operations, and whether one option provides a more consistent result than the other.
